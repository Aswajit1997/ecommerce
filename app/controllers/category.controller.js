...
categoryControllers.getAll = async (req, res) => {
	try {
		const [categories, totalCategoriesCount] = await Promise.all([
			Category.find({}, "name subCategory._id subCategory.name subCategory.fields").lean(),
...
categoryControllers.addSubCategory = async (req, res) => {
	try {
		...
		const subCategory = { _id: new mongoose.Types.ObjectId(), name };
	...
	try {
		...
		categories.forEach((category) => {
			category.subCategory.forEach((subCat) => {
				...
				formattedFields[subCat.name.toLowerCase()] = subCat?.fields?.map((field) => ({
					key: field.key,
